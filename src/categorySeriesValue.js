...
function batchPropGetter(obj) {
    return async function(keys) {
        ...
        const fetchedValues = keys.map(key => response.data.categorySeriesValue[key])
...
class CategorySeriesValue{
    ...
    static fields() {
        return ["id","createdAt","updatedAt","device","visibility","myRole","nodeCount","nodes","lastNode","cardSize","name","index","allowedValues"]
    } 
...
module.exports = {CategorySeriesValue}
